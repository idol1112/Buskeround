		...
		<div class="leftsection">
					...
					<tbody>
						<c:forEach items="${artistFan.ArtistList}" var="fList">
							...
							<tr>
								...
								<c:choose>
									<c:when test="${fList.likeOk == true}">
										<td class="left02_td"><img class="likes" data-no="${fList.user_no}" src="${pageContext.request.contextPath}/assets/image/artist/icon/heart2.png" style="width: 20px"> <span class="likes2-${fList.user_no}">${fList.likes}</span></td>
									</c:when>
									<c:otherwise>
										<td class="left02_td"><img class="likes" data-no="${fList.user_no}" src="${pageContext.request.contextPath}/assets/image/artist/icon/heart1.png" style="width: 20px"> <span class="likes2-${fList.user_no}">${fList.likes}</span></td>
									</c:otherwise>
								</c:choose>
								
								<c:choose>
									<c:when test="${fList.fanOk == true}">
										<td class="left02_td"><img class="fan" data-no="${fList.user_no}" src="${pageContext.request.contextPath}/assets/image/artist/icon/fan2.png" style="width: 20px"><span class="fan2-${fList.user_no}">${fList.fan}</span></td>
									</c:when>
									<c:otherwise>
										<td class="left02_td"><img class="fan" data-no="${fList.user_no}" src="${pageContext.request.contextPath}/assets/image/artist/icon/fan1.png" style="width: 20px"><span class="fan2-${fList.user_no}">${fList.fan}</span></td>
									</c:otherwise>
								</c:choose>
							</tr>
						</c:forEach>
			...
			<div class="containers xlarge">
				<div class="pagination">
					<ul>
						<c:if test="${artistFan.prev ==true}">
							<li><a href="${pageContext.request.contextPath}/Artist/ArtistFan?page=${artistFan.start_page_btn_no-1}&keyword=${param.keyword}">&laquo;</a></li>
						</c:if>

						<c:forEach var="page" begin="${artistFan.start_page_btn_no}" end="${artistFan.end_page_btn_no}" step="1">
							<c:choose>

								<c:when test="${param.page == page}">
									<li class="active"><a href="${pageContext.request.contextPath}/Artist/ArtistFan?page=${page}&keyword=${param.keyword}">${page}</a></li>
								</c:when>

								<c:otherwise>
									<li><a href="${pageContext.request.contextPath}/Artist/ArtistFan?page=${page}&keyword=${param.keyword}">${page}</a></li>
								</c:otherwise>

							</c:choose>
						</c:forEach>
						<c:if test="${artistFan.next ==true}">
							<li><a href="${pageContext.request.contextPath}/Artist/ArtistFan?page=${artistFan.end_page_btn_no+1}&keyword=${param.keyword}">&raquo;</a></li>
						</c:if>
