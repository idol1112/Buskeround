...
public class ArtistService {
	...
	public Map<String, Object> getArtistFan(int page, String keyword, int user_no) {
		...
		for (int i = 0; i < ArtistList.size(); i++) {
			...
			ArtistList.get(i).setArtist_no(user_no);
			String check = artistDao.checkArtist(ArtistList.get(i));
			
			if (check == null) {
				ArtistList.get(i).setFanOk(false);
			} else {
				ArtistList.get(i).setFanOk(true);
			}
		...
		for (int i = 0; i < ArtistList.size(); i++) {
			...
			ArtistList.get(i).setArtist_no(user_no);
			String check = artistDao.checkLike(ArtistList.get(i));
			
			if (check == null) {
				ArtistList.get(i).setLikeOk(false);
			} else {
				ArtistList.get(i).setLikeOk(true);
			}
	...
	// new 아티스트 리스트 가져오기
	public Map<String, Object> getArtistNew(int page, String keyword, int user_no) {
	...
	// 아티스트 장르 리스트 가져오기
	public List<UserVo> getArtistGenre(int no, int user_no) {
		
		List<UserVo> ArtistList = artistDao.getArtistGenre(no);
	...
	public List<UserVo> getFanList(int user_no) {
		...
		List<UserVo> ArtistList = artistDao.getFanList(user_no);
	...
	// 아티스트 검색 리스트
	public Map<String, Object> getArtistSearch(int page, String keyword, int user_no) {
